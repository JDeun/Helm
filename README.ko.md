...
Helm workspace를 장기적으로 다듬을 때 중요한 것은 규칙을 많이 늘리는 것이 아니라, 각 스킬을 더 좁고 명시적으로 만들고 검증 가능하게 유지하는 것입니다.
...
- release 전이나 정책 수정 후에는 `validate-manifests`를 돌릴 것
...
Helm v0.3.0은 adaptive harness governance가 중앙 registry가 아니라 스킬 로컬 manifest로 옮겨진 첫 릴리즈입니다.
